...
CodeGeneratorMIPS64::visitCompareB(LCompareB *lir)
{
    ...
    {
        if (rhs->isConstant())
            masm.cmp32Set(cond, lhs.valueReg(), Imm32(rhs->toConstant()->toBoolean()), output);
        else
            masm.cmp32Set(cond, lhs.valueReg(), ToRegister(rhs), output);
        masm.jump(&done);
    }
...
CodeGeneratorMIPS64::visitCompareBAndBranch(LCompareBAndBranch *lir)
{
    ...
    Register type = splitTagForTest(lhs);
    branchToBlock(type, ImmTag(JSVAL_TAG_BOOLEAN), mirNotBoolean, Assembler::NotEqual);

    Assembler::Condition cond = JSOpToCondition(mir->compareType(), mir->jsop());
    masm.unboxBoolean(lhs, ExtractTemp0);
    if (rhs->isConstant())
        emitBranch(ExtractTemp0, Imm32(rhs->toConstant()->toBoolean()), cond, lir->ifTrue(),
                   lir->ifFalse());
    else
        emitBranch(ExtractTemp0, ToRegister(rhs), cond, lir->ifTrue(), lir->ifFalse());
}
...
void
CodeGeneratorMIPS64::visitUDiv(LUDiv *ins)
{
    Register lhs = ToRegister(ins->lhs());
    Register rhs = ToRegister(ins->rhs());
    Register output = ToRegister(ins->output());
...
void
CodeGeneratorMIPS64::visitUMod(LUMod *ins)
{
    Register lhs = ToRegister(ins->lhs());
    Register rhs = ToRegister(ins->rhs());
    Register output = ToRegister(ins->output());
