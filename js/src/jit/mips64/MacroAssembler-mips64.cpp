...
void
MacroAssemblerMIPS64::movq(Register rd, Register rs)
{
    ma_move(rd, rs);
}
...
void
MacroAssemblerMIPS64::ma_liPatchable(Register dest, Imm32 imm)
{
...
void
MacroAssemblerMIPS64::ma_lw(Register data, Address address)
{
    ma_load(data, address, SizeWord);
}

void
MacroAssemblerMIPS64::ma_ld(Register data, Address address)
{
    ma_load(data, address, SizeDouble);
}

void
MacroAssemblerMIPS64::ma_sw(Register data, Address address)
{
    ma_store(data, address, SizeWord);
}

void
MacroAssemblerMIPS64::ma_sw(Imm32 imm, Address address)
{
    ...
    if (Imm16::IsInSignedRange(address.offset)) {
        ...
        as_daddu(SecondScratchReg, address.base, SecondScratchReg);
        as_sw(ScratchRegister, SecondScratchReg, 0);
    }
}

void
MacroAssemblerMIPS64::ma_sw(Register data, BaseIndex &address)
{
    ma_store(data, address, SizeWord);
}

void
MacroAssemblerMIPS64::ma_sd(Register data, Address address)
{
...
MacroAssemblerMIPS64::ma_ld(FloatRegister ft, Address address)
{
    ...
    if (Imm16::IsInSignedRange(address.offset) && Imm16::IsInSignedRange(off2)) {
        ...
        ma_li(ScratchRegister, Imm32(address.offset));
        as_daddu(SecondScratchReg, address.base, ScratchRegister);
        as_ldl(ScratchRegister, SecondScratchReg, 7);
...
MacroAssemblerMIPS64Compat::load32(AsmJSAbsoluteAddress address, Register dest)
{
    ...
    load32(Address(ScratchRegister, 0x0), dest);
}
...
MacroAssemblerMIPS64Compat::branchTestDouble(Condition cond, const Address &address, Label *label)
{
    MOZ_ASSERT(cond == Equal || cond == NotEqual);
    extractTag(address, SecondScratchReg);
    ma_b(SecondScratchReg, ImmTag(JSVAL_TAG_MAX_DOUBLE), label, cond);
}
...
{
    MOZ_ASSERT(cond == Equal || cond == NotEqual);
    ma_ld(ScratchRegister, Address(valaddr.base, valaddr.offset));
    ma_b(value.valueReg(), ScratchRegister, label, cond);
}
...
{
    ma_lw(dest, Address(src.base, src.offset));
}
...
MacroAssemblerMIPS64Compat::loadInt32OrDouble(const Address &src, FloatRegister dest)
{
    ...
    ma_ld(SecondScratchReg, Address(src.base, src.offset));
...
MacroAssemblerMIPS64Compat::branchTestStringTruthy(bool b, const ValueOperand &value, Label *label)
{
    ...
    ma_lw(SecondScratchReg, Address(SecondScratchReg, JSString::offsetOfLength()));
...
{
    ma_ld(scratch, Address(address.base, address.offset));
...
{
    ma_sd(val.valueReg(), Address(dest.base, dest.offset));
}
...
MacroAssemblerMIPS64Compat::storeValue(JSValueType type, Register reg, Address dest)
{
    ...
    ma_sd(SecondScratchReg, Address(dest.base, dest.offset));
}
...
MacroAssemblerMIPS64Compat::storeValue(const Value &val, Address dest)
{
    ...
    ma_sd(ScratchRegister, Address(dest.base, dest.offset));
}
...
{
    ma_ld(val.valueReg(), Address(src.base, src.offset));
}
...
MacroAssemblerMIPS64Compat::pushValue(const Address &addr)
{
    ...
    ma_dsubu(StackPointer, StackPointer, Imm32(sizeof(Value)));
    // Store type and payload.
    ma_ld(ScratchRegister, Address(addr.base, addr.offset));
    ma_sd(ScratchRegister, Address(StackPointer, 0));
}
                                                     ...
                                                     Register temp, Label *label)
{
    ...
    branchPtrInNurseryRange(cond, value.valueReg(), temp, label);
